...
export function genRoutes(routes: RouteRecordRaw[], parent?: MenuDataItem) {
  ...
  routes.forEach((route) => {
    ...
    if(!hideMenuList.includes(item.path)) menuData.push(item)
  })
  return menuData
}
...
export function generateFlatRoutes(routes: RouteRecordRaw[]) {
  ...
  let errorPage =  {
        ...
        component: () => import('@/pages/exception/notArrow.vue'),
      }
  // 拿到拉平后的路由，然后统一添加一个父级的路由,通过这层路由实现保活的功能
  const parentRoute: RouteRecordRaw = {
    path: '/',
    redirect: flatRoutesList?.[0]?.path || ROOT_ROUTE_REDIRECT_PATH,
