...
const columns: CustomColumnsType<any>[] = [
  ...
  },
  {
    title: '行业资质是否必填',
    dataIndex: 'config',
    mapName: (row:any) => row.text.needIndustryQualification ? '是' : '否',
    width: 130
  }
]
const formConf = shallowRef<FormConfig[]>([
  {
    ...
    slots: 'selects',
    type: 'select',
    // options: basePage?.value?.parentList,
...
<template>
  <div>
  <page-container>
    <BasePage ref="basePage" type="life_industry" :formConf="formConf" :columns="columns" :maxLevel="2"></BasePage>
  </page-container>
  </div>
</template>
